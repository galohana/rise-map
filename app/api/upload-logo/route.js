/*
  Logo upload — server-side only.
  Password-gated (x-admin-password). Uploads to the public Supabase Storage
  bucket "logos" using the service key, returns the public URL.
*/
...
export async function POST(request) {
  ...
  if (file.size > 3 * 1024 * 1024) return Response.json({ error: 'הקובץ גדול מדי (מקסימום 3MB)' }, { status: 400 })
