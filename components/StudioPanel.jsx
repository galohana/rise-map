...
/* ── Fullscreen lightbox ─────────────────────────────────────────────── */
function GalleryLightbox({ images, color, onClose }) {
    ...
    >
      ...
      <button onClick={onClose} aria-label="סגור"
        className="absolute top-5 right-5 w-10 h-10 rounded-full flex items-center justify-center
                   text-white/70 hover:text-white border border-white/15 hover:border-white/30 transition-all"
      ...
      <motion.div
        ...
        className="relative w-full px-6" style={{ maxWidth: '360px' }}
        onClick={e => e.stopPropagation()}
      >
        <div className="relative rounded-2xl overflow-hidden"
          style={{ aspectRatio: '3/4', boxShadow: `0 28px 80px rgba(0,0,0,0.65), 0 0 0 1px ${color}30` }}>
          ...
          <div className="flex items-center justify-between mt-4 px-1">
            <button onClick={prev} aria-label="הקודם"
              className="w-10 h-10 rounded-full flex items-center justify-center text-white/60
                         hover:text-white border border-white/15 hover:border-white/35 transition-all text-xl"
...
function SocialLinks({ studio }) {
  const lat = Number(studio.lat), lng = Number(studio.lng)
  ...
  if (studio.google_url)
    icons.push({ key: 'g', href: studio.google_url, label: 'Google', bg: '#fff', ring: true,
...
export default function StudioPanel({ studio, onClose }) {
  ...
  const mobileStyle = {
    position: 'fixed', right: 0, top: '104px', width: '82vw', height: '58vh',
    borderRadius: '20px 0 0 20px', zIndex: 1101,
  }
  const desktopClass = 'fixed z-[1101] top-3 bottom-3 right-3 w-[400px] rounded-[20px]'

  return (
    <>
      {!isMobile && (
        <motion.div className="fixed inset-0 z-[1100] bg-black"
      ...
      >
              ...
              <div className="flex-1 min-w-0 space-y-2">
                {(studio.address || studio.city) && (
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-heebo text-[13px] text-zinc-700 leading-tight truncate">
                      {studio.address || studio.city}
                    </span>
                  </div>
                )}
                {(hasYears || hasSpecialty) && (
                  <p className="font-heebo text-[12.5px] text-zinc-500 leading-snug">
                    {hasYears && <span>{studio.years_experience} שנות ניסיון</span>}
          ...
          {hasDescription && (
            <motion.div variants={item}>
              <p className="font-heebo text-[13px] leading-relaxed italic text-[#6B5E4F] px-1"
                 style={{ borderInlineStart: `2px solid ${color}40`, paddingInlineStart: '10px' }}>
        ...
        {hasUrl && (
          <div className="flex-shrink-0 p-4 border-t border-zinc-100 bg-white/50">
            <motion.a href={studio.url} target="_blank" rel="noopener noreferrer"
