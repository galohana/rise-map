...
function Gate({ onUnlock }) {
  ...
  const submit = async (e) => {
    ...
    setPw(pw)
    try {
      await adminApi.list()      // validates password server-side
      onUnlock()
    } catch (e) {
      clearPw()
  ...
  return (
    <div className="h-[100dvh] overflow-y-auto flex items-center justify-center bg-[#FAF8F5] px-5 py-8" dir="rtl">
      ...
      >
        ...
        <Link to="/" className="block text-center mt-4 text-[12px] font-heebo text-[#A09A96] hover:text-[#1C1916] transition">
          ← חזרה למפה
        </Link>
      </motion.form>
    </div>
...
export default function AdminPage() {
  ...
  const [editing, setEditing] = useState(null)   // studio object or {} for new
  ...
  // Auto-resume session if password already stored
  useEffect(() => {
    if (getPw()) {
      adminApi.list()
      ...
      <header className="sticky top-0 z-40 bg-[#FAF8F5]/95 backdrop-blur border-b border-[#E8E2DC]">
        <div className="max-w-2xl mx-auto px-5 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link to="/" className="text-[12px] font-heebo text-[#A09A96] hover:text-[#1C1916] transition">מפה</Link>
